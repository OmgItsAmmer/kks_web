...
router.get('/', (0, error_middleware_ts_1.asyncHandler)(async (req, res) => {
    const categories = await category_repository_ts_1.categoryRepository.findAll();
    // Fetch images
    const categoryIds = categories.map((c) => c.category_id);
    const images = await image_service_ts_1.imageService.getMainImagesForEntities(categoryIds, 'categories');
    const categoriesWithImages = categories.map((category) => ({
        ...category,
        imageUrl: images.get(category.category_id) || null,
    }));
    return (0, response_ts_1.sendSuccess)(res, categoriesWithImages);
}));
/**
...
router.get('/featured', (0, error_middleware_ts_1.asyncHandler)(async (req, res) => {
    const categories = await category_repository_ts_1.categoryRepository.findFeatured();
    // Fetch images
    const categoryIds = categories.map((c) => c.category_id);
    const images = await image_service_ts_1.imageService.getMainImagesForEntities(categoryIds, 'categories');
    const categoriesWithImages = categories.map((category) => ({
...
router.get('/:id', (0, validation_middleware_ts_1.validate)({ params: validation_middleware_ts_1.schemas.idParam }), (0, error_middleware_ts_1.asyncHandler)(async (req, res) => {
    ...
    const category = await category_repository_ts_1.categoryRepository.findById(categoryId);
    if (!category) {
        return (0, response_ts_1.sendNotFound)(res, 'Category not found');
    }
    const imageUrl = await image_service_ts_1.imageService.getMainImageUrl(categoryId, 'categories');
