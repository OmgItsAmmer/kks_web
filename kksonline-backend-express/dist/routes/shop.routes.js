 ...
 * @access  Public
 */
router.get('/config', (0, error_middleware_ts_1.asyncHandler)(async (req, res) => {
    const config = await shop_repository_ts_1.shopRepository.getConfig();
    if (!config) {
 ...
 * @access  Public
 */
router.get('/app-version', (0, error_middleware_ts_1.asyncHandler)(async (req, res) => {
    const version = await shop_repository_ts_1.shopRepository.getLatestAppVersion();
    if (!version) {
        return (0, response_ts_1.sendSuccess)(res, {
            version: null,
            forceUpdate: false,
 ...
 * @access  Public
 */
router.post('/check-version', (0, error_middleware_ts_1.asyncHandler)(async (req, res) => {
    ...
    const result = await shop_repository_ts_1.shopRepository.checkAppVersion(currentVersion);
    return (0, response_ts_1.sendSuccess)(res, result);
}));
/**
 ...
 * @access  Public
 */
router.get('/shipping-enabled', (0, error_middleware_ts_1.asyncHandler)(async (req, res) => {
    const isEnabled = await shop_repository_ts_1.shopRepository.isShippingEnabled();
    return (0, response_ts_1.sendSuccess)(res, { isShippingEnabled: isEnabled });
}));
