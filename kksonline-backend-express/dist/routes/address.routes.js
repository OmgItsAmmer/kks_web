...
router.get('/', (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    const addresses = await address_repository_1.addressRepository.findByCustomerId(req.customerId);
...
router.get('/:id', (0, validation_middleware_1.validate)({ params: validation_middleware_1.schemas.idParam }), (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    if (address.customer_id !== req.customerId) {
        throw new errors_1.ForbiddenError('Access denied to this address');
    }
 ...
 * @access  Private
 */
router.post('/', (0, validation_middleware_1.validate)({ body: validation_middleware_1.schemas.address }), (0, error_middleware_1.asyncHandler)(async (req, res) => {
    if (!req.customerId) {
        return (0, response_1.sendError)(res, 'Unauthorized', 401);
    }
    ...
    const address = await address_repository_1.addressRepository.create(addressData);
    return (0, response_1.sendCreated)(res, address, 'Address created successfully');
}));
/**
...
router.put('/:id', (0, validation_middleware_1.validate)({
    ...
    body: validation_middleware_1.schemas.address.partial(),
    ...
    const addressId = parseInt(req.params.id, 10);
    // Verify ownership
    const belongs = await address_repository_1.addressRepository.belongsToCustomer(addressId, req.customerId);
    if (!belongs) {
        throw new errors_1.ForbiddenError('Access denied to this address');
    }
    const { fullName, shippingAddress, city, postalCode, phoneNumber, country, latitude, longitude, place_id, formatted_address } = req.body;
    ...
    const address = await address_repository_1.addressRepository.update(addressId, updates);
