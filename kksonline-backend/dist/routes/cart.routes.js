 ...
 * @access  Private
 */
router.post('/', validate({ body: schemas.cartItem }), asyncHandler(async (req, res) => {
    if (!req.customerId) {
        return sendError(res, 'Unauthorized', 401);
    }
    ...
    if (!limitCheck.allowed) {
        return sendError(res, `Cannot add ${quantity} items. Maximum allowed: ${limitCheck.maxAllowedQuantity}, ` +
    ...
    if (!canAdd) {
        return sendError(res, 'Insufficient stock for this item', 400, 'INSUFFICIENT_STOCK');
    }
 ...
 * @access  Private
 */
router.delete('/:cartId', validate({ params: z.object({ cartId: z.string().transform(Number) }) }), asyncHandler(async (req, res) => {
    if (!req.customerId) {
        return sendError(res, 'Unauthorized', 401);
    }
    const cartId = parseInt(req.params.cartId, 10);
 ...
 * @access  Private
 */
router.post('/apply-adjustments', asyncHandler(async (req, res) => {
    if (!req.customerId) {
        return sendError(res, 'Unauthorized', 401);
    }
 ...
 * @access  Private
 */
router.post('/transfer-to-kiosk', asyncHandler(async (req, res) => {
    if (!req.customerId) {
        return sendError(res, 'Unauthorized', 401);
    }
