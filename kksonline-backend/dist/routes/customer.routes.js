 ...
 * @access  Private
 */
router.put('/profile', authenticate, validate({ body: schemas.customerUpdate }), asyncHandler(async (req, res) => {
    if (!req.customerId) {
        return sendError(res, 'Unauthorized', 401);
    }
 ...
 * @access  Private
 */
router.post('/profile-picture', authenticate, upload.single('image'), asyncHandler(async (req, res) => {
    if (!req.customerId) {
        return sendError(res, 'Unauthorized', 401);
    }
    if (!req.file) {
    ...
    const result = await imageService.updateMainImage(req.file.buffer, 'customers', req.customerId, req.file.originalname);
    return sendSuccess(res, {
        imageUrl: result.url,
 ...
 * @access  Private
 */
router.delete('/profile-picture', authenticate, asyncHandler(async (req, res) => {
    if (!req.customerId) {
        return sendError(res, 'Unauthorized', 401);
    }
